...
const CodingIcon = () => (
  <img src="/images/3D_Coding.png" alt="3D" className="w-full h-full object-contain" />
);
...
const Landing = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from(".hero-title span", { y: 60, opacity: 0, duration: 1, stagger: 0.1 })
        .from(".hero-subtext", { y: 20, opacity: 0, duration: 0.8 }, "-=0.6")
        .from(".hero-btns", { y: 20, opacity: 0, duration: 0.6 }, "-=0.4")
        .from(iconRef.current, { scale: 0.8, opacity: 0, duration: 1.2, ease: "back.out(1.7)" }, "-=0.8")
        .from(".orbit-ring", { scale: 0, opacity: 0, stagger: 0.2 }, "-=1");

      // Floating animations
      gsap.to(iconRef.current, { y: -15, duration: 3, repeat: -1, yoyo: true, ease: "sine.inOut" });
  ...
  return (
    <section ref={containerRef} className="relative w-full min-h-[calc(100vh-80px)] flex items-center overflow-hidden bg-white py-12 lg:py-0">
      
      {/* 1. Background Grid - Standardized Opacity */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          {/* LEFT CONTENT - Standard Typography Scales */}
          <div className="lg:col-span-6 text-center lg:text-left space-y-6 md:space-y-8">
            <h1 className="hero-title text-4xl sm:text-5xl md:text-6xl xl:text-7xl font-extrabold tracking-tight text-slate-900 leading-[1.1]">
              ...
              <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600">Skills</span>
            </h1>
          ...
          <div className="lg:col-span-6 flex justify-center relative py-12 lg:py-20">
            ...
            {/* Main Icon Container */}
            <div ref={iconRef} className="relative w-64 h-64 sm:w-80 sm:h-80 md:w-[420px] md:h-[420px] z-10">
              <CodingIcon />
              
              {/* Floating Status Chips - Responsive Hidden on small mobile */}
              <div className="absolute -top-4 -right-4 hidden sm:flex items-center gap-2 bg-white p-3 rounded-2xl shadow-xl border border-slate-50">
                <div className="p-1.5 bg-blue-50 text-blue-600 rounded-lg"><Zap size={16} /></div>
                <span className="text-xs font-bold text-slate-700">Live Support</span>
              </div>
              ...
              <div className="absolute top-1/2 -left-8 hidden sm:flex items-center gap-2 bg-white p-3 rounded-2xl shadow-xl border border-slate-50">
                <div className="p-1.5 bg-purple-50 text-purple-600 rounded-lg"><Brain size={16} /></div>
                <span className="text-xs font-bold text-slate-700">Expert Led</span>
              </div>
            </div>
